...
export default function EmployerDashboard() {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setShowForm(false);
      setForm({ title: '', description: '', location: '', salaryMin: '', salaryMax: '', jobRegime: 'FULL_TIME', jobPeriod: '', experience: 'NONE', applicationMethod: 'CV_ONLY' });
      fetchJobs();
    } catch (err) {
  ...
  return (
    <div className='min-h-screen bg-slate-50'>
      ...
      <div className='max-w-4xl mx-auto py-10 px-4'>
        ...
        {message && <p className='text-teal-600 mb-4 font-medium'>{message}</p>}
        ...
        {showForm && (
          <form onSubmit={handleCreate} className='bg-white rounded-2xl shadow-sm p-6 mb-8 space-y-4'>
            ...
            <textarea required placeholder='Job Description' value={form.description} rows={4}
            ...
            <div className='grid grid-cols-2 gap-4'>
              <input placeholder='Location (optional)' value={form.location}
                ...
                className='w-full border border-slate-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-teal-500' />
              <input placeholder='Job Period (e.g. 1 month)' value={form.jobPeriod}
                onChange={e => setForm(p => ({ ...p, jobPeriod: e.target.value }))}
                className='w-full border border-slate-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-teal-500' />
              <input required type='number' placeholder='Min Salary (GEL)' value={form.salaryMin}
                onChange={e => setForm(p => ({ ...p, salaryMin: e.target.value }))}
                className='w-full border border-slate-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-teal-500' />
              <input type='number' placeholder='Max Salary (GEL)' value={form.salaryMax}
                onChange={e => setForm(p => ({ ...p, salaryMax: e.target.value }))}
                className='w-full border border-slate-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-teal-500' />
            </div>
            <div className='grid grid-cols-3 gap-4'>
              <select value={form.jobRegime} onChange={e => setForm(p => ({ ...p, jobRegime: e.target.value }))}
                className='w-full border border-slate-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-teal-500'>
              ...
              <select value={form.applicationMethod} onChange={e => setForm(p => ({ ...p, applicationMethod: e.target.value }))}
                className='w-full border border-slate-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-teal-500'>
            ...
            <div key={job.id} className='bg-white rounded-xl shadow-sm p-6'>
              <div className='flex justify-between items-start'>
                <div>
                  ...
                  <p className='text-slate-700 font-medium mt-1'>{job.salaryMin.toLocaleString()} GEL</p>
                </div>
              ...
              <div className='flex gap-3 mt-4'>
                ...
                <button onClick={() => handleDelete(job.id)} className='text-sm text-red-400 hover:underline'>
                  Delete
                </button>
              </div>
            </div>
