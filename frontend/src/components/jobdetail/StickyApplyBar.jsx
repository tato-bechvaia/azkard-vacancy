...
export default function StickyApplyBar({ job, user, applied, isSaved, onApply, onSave, onShare }) {
  // Employers never see the apply bar
  ...
  return (
    <>
      {/* ── Desktop: sticky below hero, floats at navbar bottom on scroll ── */}
      <div className={`hidden md:flex items-center justify-between gap-4 rounded-2xl px-6 py-3.5 mb-3 sticky top-[5.5rem] z-10 ${barClasses}`}>
        ...
        <div className='flex items-center gap-2 flex-shrink-0'>
          <Button
            ...
            iconOnly
            onClick={onSave}
            className={isSaved ? 'text-brand-400 hover:text-brand-300' : 'text-text-muted hover:text-text-secondary'}
      ...
      >
        <Button
          ...
          iconOnly
          onClick={onSave}
          className={isSaved ? 'text-brand-400' : 'text-text-muted'}
        >
          <BookmarkIcon filled={isSaved} />
        </Button>
        <Button
